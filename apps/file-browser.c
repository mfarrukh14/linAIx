			...
			if (S_ISDIR(statbuf.st_mode)) {
				...
				if (is_desktop_background && has_extension(f, ".launcher")) {
					...
					if (has_extension(f, ".c")) {
						...
						sprintf(f->icon, "file");
						sprintf(f->filetype, "linAIx Rich Text Document");
...
static void _menu_action_about(struct MenuEntry * entry) {
	...
	strcat(about_cmd, "about \"About File Browser\" /usr/share/icons/48/folder.png \"linAIxOS File Browser\" \"© 2018-2026 K. Lange\n-\nPart of linAIxOS, which is free software\nreleased under the NCSA/University of Illinois\nlicense.\n-\n%https://linAIxos.org\n%https://github.com/klange/linAIxos\" ");
	char coords[100];
...
static void launch_application(char * app) {
	if (!fork()) {
		...
		if (!shell || !*shell) shell = "/bin/esh";
		char * args[] = {shell, "-c", tmp, NULL};
		execvp(args[0], args);
		exit(1);
	}
}
...
static void open_file(struct File * f) {
	if (f->type == 1) {
		...
		if (!strcmp(f->launcher, "SELF")) {
			...
			/* Desktop launcher entries already contain complete commands. */
			sprintf(tmp, "%s", f->launcher);
		} else {
...
int main(int argc, char * argv[]) {
	...
	menu_insert(context_menu, menu_create_normal("utilities-terminal","exec terminal","Open Terminal",launch_application_menu));
	...
	menu_insert(directory_context_menu, menu_create_normal("utilities-terminal","exec terminal","Open Terminal",launch_application_menu));
