...
static void handle_syscall(pid_t pid, struct URegs * r) {
	...
	switch (uregs_syscall_num(r)) {
		...
		case SYS_SYSFUNC:
			switch (uregs_syscall_arg1(r)) {
				C(linAIx_SYS_FUNC_SYNC);
				C(linAIx_SYS_FUNC_LOGHERE);
				C(linAIx_SYS_FUNC_KDEBUG);
				C(linAIx_SYS_FUNC_INSMOD);
				C(linAIx_SYS_FUNC_SETHEAP);
				C(linAIx_SYS_FUNC_MMAP);
				C(linAIx_SYS_FUNC_THREADNAME);
				C(linAIx_SYS_FUNC_SETGSBASE);
				C(linAIx_SYS_FUNC_NPROC);
				C(linAIx_SYS_FUNC_CLEARICACHE);
				C(linAIx_SYS_FUNC_MUNMAP);
				default: int_arg(uregs_syscall_arg1(r)); break;
			} COMMA;
			pointer_arg(uregs_syscall_arg2(r));
			break;
