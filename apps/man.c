...
#if !defined(linAIx_MAN_CROSS_VIEWER)
#define MORE_CMD "more -rP'%s(%s)' --stay --alt"
#define MAN_FMT  "/usr/share/man/man%s/%s.%s"
#define ROFF_CMD "roff"
#else
#define MORE_CMD "linAIx-more -rP'%s(%s)' --stay --alt"
#define MAN_FMT linAIx_MAN_CROSS_DIR "/usr/share/man/man%s/%s.%s"
#define ROFF_CMD "linAIx-roff"
#endif
...
int main(int argc, char * argv[]) {
	 ...
	 * if we're building for linAIxOS and it is set - or the default
	 * list that we stole from an Ubuntu configuration. */
	char * section_list =
#if defined(__linAIx__)
		getenv("MANSECT");
