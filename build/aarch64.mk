...
EMU_ARGS += -name "linAIxOS ${ARCH}"

EMU_RAMDISK = -fw_cfg name=opt/org.linAIxos.initrd,file=ramdisk.igz
EMU_KERNEL  = -fw_cfg name=opt/org.linAIxos.kernel,file=misaka-kernel
...
shell: system
	@${QEMU} -M ${EMU_MACH} -m ${RAM} -smp ${SMP} -cpu ${EMU_CPU} -no-reboot -display none -serial mon:stdio -d guest_errors \
		...
		-name "linAIxOS ${ARCH}" -kernel bootstub \
		-append "root=/dev/ram0 migrate start=--headless" ${EMU_RAMDISK} ${EMU_KERNEL} \
		-fw_cfg name=opt/org.linAIxos.gettyargs,string="-a local /dev/ttyS0 115200 ${TERM}"
