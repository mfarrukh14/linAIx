...
fast: system
	${EMU} ${EMU_ARGS} -cdrom image.iso \
		-fw_cfg name=opt/org.linAIxos.bootmode,string=normal \

run-vga: system
	${EMU} ${EMU_ARGS} -cdrom image.iso \
		-fw_cfg name=opt/org.linAIxos.bootmode,string=vga \
...
shell: system
	@${EMU} -M ${EMU_MACH} -m $(RAM) -smp $(SMP) ${EMU_KVM} -cdrom image.iso \
		-nographic -no-reboot -audiodev none,id=id -serial null -serial mon:stdio \
		-fw_cfg name=opt/org.linAIxos.gettyargs,string="-a local /dev/ttyS1 115200 ${TERM}" \
		-fw_cfg name=opt/org.linAIxos.bootmode,string=headless
