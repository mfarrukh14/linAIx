...
int linAIx_auth_check_pass(char * user, char * pass) {
...
void linAIx_auth_set_vars(void) {
	int uid = getuid();
...
void linAIx_auth_set_groups(uid_t uid) {
...
void linAIx_set_credentials(uid_t uid) {
	linAIx_auth_set_groups(uid);
	setgid(uid);
	setuid(uid);
