...
long sys_sysfunc(long fn, char ** args) {
	...
	switch (fn) {
		...
		case linAIx_SYS_FUNC_CLEARICACHE:
			#ifdef __aarch64__
			PTR_VALIDATE(&args[0]);
			PTR_VALIDATE(&args[1]);
		...
		case linAIx_SYS_FUNC_THREADNAME: {
		...
		case linAIx_SYS_FUNC_SETGSBASE:
